...
def agregar_quimico(bodega_id):
    bodega = Bodega.query.get_or_404(bodega_id)
    if not tecnico_puede_acceder_a_bodega(bodega):
        print("🧪 Técnico actual:", current_user.id, current_user.email)
        print("📦 Bodega actual:", bodega.id, bodega.nombre)
        print("📋 Bodegas asignadas:", [b.id for b in current_user.bodegas_asignadas.all()])
        print("✅ Tiene permiso:", bodega in current_user.bodegas_asignadas.all())
