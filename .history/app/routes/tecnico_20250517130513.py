...
from app.models import (
    Bodega, Huerto, Recomendacion, Quimico, FormularioTarea, ChecklistItem, db,
    ActividadHuerto  # <-- NUEVO: Modelo de actividades del huerto
)
from app.forms import (
    QuimicoForm, ResponderFormularioForm, ChecklistItemForm,
    ActividadHuertoForm   # <-- NUEVO: Formulario de actividades del huerto
)
...
def tecnico_puede_acceder_a_huerto(huerto):
    # Asume que tienes una relación huertos_asignados (o filtra por responsable)
    return huerto in current_user.huertos_asignados or huerto.responsable_id == current_user.id
...
def bitacora_huerto(huerto_id):
    ...
    query = ActividadHuerto.query.filter_by(huerto_id=huerto.id)
    if anio:
        query = query.filter(db.extract('year', ActividadHuerto.fecha) == anio)
    if tipo:
        query = query.filter_by(tipo=tipo)
    actividades = query.order_by(ActividadHuerto.fecha.desc()).all()
    lista_anios = sorted({a.fecha.year for a in huerto.actividades}, reverse=True)
    return render_template(
        'bitacora_huerto.html',
        huerto=huerto,
...
def registrar_actividad_huerto(huerto_id):
    ...
    if form.validate_on_submit():
        nueva_actividad = ActividadHuerto(
            ...
            # fotos: pendiente si quieres manejar imágenes
        )
...
@tecnico_bp.route('/actividad/<int:actividad_id>')
@login_required
def detalle_actividad(actividad_id):
    actividad = ActividadHuerto.query.get_or_404(actividad_id)
    huerto = actividad.huerto
    if not tecnico_puede_acceder_a_huerto(huerto):
