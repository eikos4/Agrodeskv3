...
def bitacora_huerto(huerto_id):
    ...
    actividades = huerto.actividades  # trae todas las actividades de ese huerto
    return render_template('admin/bitacora_huerto.html', huerto=huerto, actividades=actividades)

  # <- agrega esto arriba si no está
    ...
    if form.validate_on_submit():
        try:
            actividad = ActividadHuerto(
                ...
                responsable=form.responsable.data,         # o current_user.name si lo prefieres
                ...
                fotos='',  # Si gestionas archivos, pon la ruta aquí
                observaciones=form.observaciones.data
            )
            db.session.add(actividad)  # << ESTA LÍNEA ES CLAVE
            db.session.commit()
...
@admin_bp.route('/actividad/<int:actividad_id>')
@login_required
@admin_required
def detalle_actividad(actividad_id):
    from app.models import ActividadHuerto
    actividad = ActividadHuerto.query.get_or_404(actividad_id)
    return render_template('admin/detalle_actividad.html', actividad=actividad)
...
def inject_timeline_utils():
      def tipo_color(tipo):
          ...
          return color_map.get(tipo, "bg-otra")
      def tipo_icono(tipo):
          icon_map = {
          ...
          return icon_map.get(tipo, "bi-gear")
