...
def ver_recomendaciones():
    ...
    huertos = Huerto.query.filter_by(tecnico_id=current_user.id).all()  # Ajusta la consulta según tu modelo
    bodegas = Bodega.query.filter_by(tecnico_id=current_user.id).all()  # Ajusta la consulta según tu modelo
