...
def tecnico_puede_acceder_a_huerto(huerto):
    # Acceso solo si está asignado al técnico
...
def tecnico_dashboard():
    bodegas = list(set(current_user.bodegas_asignadas.all()) | set(Bodega.query.filter_by(responsable_id=current_user.id).all()))
...
def bitacora_huerto(huerto_id):
    ...
    lista_anios = sorted(set(a.fecha.year for a in actividades if a.fecha))
    ...
    if anio_seleccionado:
        actividades_filtradas = [a for a in actividades_filtradas if a.fecha and a.fecha.year == anio_seleccionado]
...
def registrar_actividad_huerto(huerto_id):
    ...
    if form.validate_on_submit():
        try:
            # --- Crea y guarda la actividad real (esto es lo que suele faltar) ---
            ...
            actividad = ActividadHuerto(
                ...
                fotos='',  # Si luego implementas uploads
