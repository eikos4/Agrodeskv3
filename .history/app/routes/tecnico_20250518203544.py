...
# -------------------------------
# Recomendaciones (única función, pro)
# -------------------------------
...
def bitacora_huerto(huerto_id):
    ...
    # Chequea que el técnico realmente tiene acceso
    if huerto not in current_user.huertos_asignados:
        flash("No tienes acceso a este huerto.", "danger")
        return redirect(url_for('tecnico.mis_huertos'))
    
    actividades = huerto.actividades  # Trae todas las actividades
    # Si usas filtros, agrégalos aquí igual que en el admin
    lista_anios = sorted(list(set([a.fecha.year for a in actividades])))
    anio_seleccionado = request.args.get('anio', type=int)
    tipo_seleccionado = request.args.get('tipo')

    # Filtros (idéntico al admin si quieres)
    actividades_filtradas = actividades
    if anio_seleccionado:
        actividades_filtradas = [a for a in actividades_filtradas if a.fecha.year == anio_seleccionado]
    if tipo_seleccionado:
        actividades_filtradas = [a for a in actividades_filtradas if a.tipo == tipo_seleccionado]

    return render_template(
        'tecnico/bitacora_huerto.html',
        huerto=huerto,
...
def registrar_actividad_huerto(huerto_id):
    ...
    if form.validate_on_submit():
        try:
            # --- Aquí debes crear y guardar la actividad real, ejemplo:
