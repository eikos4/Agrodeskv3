...
def ver_quimicos(bodega_id):
    ...
    # Ahora sí, obtiene los químicos asociados
    quimicos = bodega.quimicos  # O usa .all() si es relación dinámica, pero por defecto lazy='select' entrega lista
...
def agregar_quimico(bodega_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            print(f"Químico agregado: {nuevo_quimico.nombre} en bodega {bodega.id}")
...
def completar_recomendacion(reco_id):
    ...
    return redirect(url_for('tecnico/formulario_responder.htm'))
